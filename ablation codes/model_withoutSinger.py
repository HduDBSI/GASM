...
class SessionGraph(Module):
    ...
    def compute_scores(self, usr_embedding, music_embedding, album_embedding):  # 传入seq_hidden
        ...
        if not self.nonhybrid:
            st = self.linear_transform1(torch.cat([a, c], 1))  # short-term
            dm = self.linear_transform2(
                torch.cat([last_music_embedding, last_album_embedding], 1))  # dynamic
...
def forward(model, i, data):  # 传入Data数据
    ...
    return targets, model.compute_scores(usr_embedding, music_embedding, album_embedding)
