...
class SessionGraph(Module):
    ...
    def compute_scores(self, usr_embedding, music_embedding, artist_embedding):  # 传入seq_hidden

        ht = usr_embedding.view(usr_embedding.shape[0], usr_embedding.shape[1], 1)  # batch_size x latent_size x 1
        ...
        if not self.nonhybrid:
            st = self.linear_transform1(torch.cat([a, b], 1))  # short-term
            dm = self.linear_transform2(
                torch.cat([last_music_embedding, last_artist_embedding], 1))  # dynamic
...
def forward(model, i, data):  # 传入Data数据
    ...
    return targets, model.compute_scores(usr_embedding, music_embedding, artist_embedding)
