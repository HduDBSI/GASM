...
def data_input(opt):
    ...
    # print('TRAIN——the frequency of missing artist{},the frequency of missing album{}'.format(train_missed_artist,
    #                                                                                          train_missed_album))
    # print('TRAIN——the number of missing artist{},the number of missing album{}'.format(len(train_missed_artist_set),
    #                                                                                    len(train_missed_album_set)))
    # print('TEST———the frequency of missing artist{},the frequency of missing album{}'.format(test_missed_artist,test_missed_album))
    # print('TEST———the number of missing artist{},the number of missing album{}'.format(len(test_missed_artist_set),len(test_missed_album_set)))
    # print('All listening records length:{}'.format(train_lenth+test_lenth))
    # print('All user num:{}'.format(len(user_set)))
    ...
    '''
    # 统计下标id
    music_slice,aritst_slice,album_slice=[],[],[]
    for i in range(len(train_music)):
        music_slice+=train_music[i]
        aritst_slice+=train_artist[i]
        album_slice+=train_album[i]
    for i in range(len(test_music)):
        music_slice += test_music[i]
        aritst_slice += test_artist[i]
        album_slice += test_album[i]
    music_slice=np.asarray(music_slice)
    aritst_slice = np.asarray(aritst_slice)
    album_slice = np.asarray(album_slice)
    print(music_slice.min(),music_slice.max())
    print(aritst_slice.min(), aritst_slice.max())
    print(album_slice.min(), album_slice.max())
    '''
    # ---------------------------------------------------------------
    ...
    # artist2music_dic = defaultdict(set)  # 存储每个艺术家id对应的音乐-用于artist推荐
    # for music, artist in zip(train_music[1], train_artist[2]):
    #     artist2music_dic[artist].add(music)
    # for music, artist in zip(test_music[1], test_artist[2]):
    #     artist2music_dic[artist].add(music)
    # artis_list = sorted(artist2music_dic.keys())# 根据artistid进行排序，映射到一个0~n_artist的下标的二维数组中
    # artist2music_list = list()
    # for a in artis_list:
    #     artist2music_list.append(artist2music_dic[a])

    # print("Total train records lines: {}".format(len(train_data[0])))
    # print("Total train lenth: {}".format(train_lenth))
    # print("Total test records lines: {}".format(len(test_data[0])))
    # print("Total test lenth: {}".format(test_lenth))
...
def build_graph(music_seq, artist_seq, album_seq):
    ...
    for music_slice, artist_slice, album_slice in zip(music_seq, artist_seq, album_seq):
        a = np.unique(music_slice + artist_slice + album_slice)  # album中包含0表示不存在
        items.append(a)
