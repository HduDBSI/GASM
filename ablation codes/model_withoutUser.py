...
class SessionGraph(Module):
    def __init__(self, opt, n_usr, n_album, n_artist, n_music):
        ...
        self.linear_three_1 = nn.Linear(self.hidden_size, 1, bias=False)
        self.linear_three_2 = nn.Linear(self.hidden_size, 1, bias=False)
        self.linear_three_3 = nn.Linear(self.hidden_size, 1, bias=False)
    ...
    def compute_scores(self, music_embedding, artist_embedding, album_embedding):  # 传入seq_hidden
        ...
        # 线性变换代替user
        alpha = self.linear_three_1(torch.tanh(q1_1 + q1_2))
        beta = self.linear_three_2(torch.tanh(q2_1 + q2_2))
        delta = self.linear_three_3(torch.tanh(q3_1 + q3_2))
        ...
        all = self.linear_transform3(torch.cat([st, dm], 1))
    ...
    def forward(self, item, A):
        h1 = self.item_embedding(item)
        hidden = self.gnn(A, h1)
        return hidden
...
def forward(model, i, data):  # 传入Data数据
    ...
    hidden = model(items, A)  # 获取GNN的节点特征表示
    ...
    return targets, model.compute_scores(music_embedding, artist_embedding, album_embedding)
