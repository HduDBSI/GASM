...
def build_graph(music_seq, artist_seq, album_seq):
    ...
    for music_slice, artist_slice, album_slice in zip(music_seq, artist_seq, album_seq):
        ...
        items.append(a)
        len_list.append(a.shape[0])  # +1为用户节点的占位
    ...
    for i in range(len(items)):
        music_alias_input.append([np.where(items[i] == m)[0][0] for m in music_seq[i]])
        artist_alias_input.append([np.where(items[i] == m)[0][0] for m in artist_seq[i]])
        album_alias_input.append([np.where(items[i] == m)[0][0] for m in album_seq[i]])
        mask = []
        ...
        for music, artist, album in zip(music_seq[i], artist_seq[i], album_seq[i]):
            if pre!= -1:
                pre_music_pos=np.where(items[i] == pre)[0][0]
            now_music_pos=np.where(items[i] == music)[0][0]
            artist_pos=np.where(items[i] == artist)[0][0]
            album_pos=np.where(items[i] == album)[0][0]
