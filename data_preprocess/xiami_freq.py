...
def generate_data(freq_n_item, top_n_user, min_length, max_length, data, BASE_DIR, DATA_SOURCE,partition):
    ...
    tr_user_item_time_record = os.path.join(BASE_DIR, DATA_SOURCE,
                                            'tr-user-item-time-freq{}-min{}-max{}-{}-{}'.format(freq_n_item, min_length,
                                                                                             max_length,
                                                                                             NORM_METHOD,int(partition*10)) + '.lst')
    # next item
    ...
    te_baseline_user_old_item_time_record = os.path.join(BASE_DIR, DATA_SOURCE,
                                                         'baseline_te_old_freq{}_partition{}.lst'.format(freq_n_item,int(partition*10)))
    ...
    index2item_path = os.path.join(BASE_DIR, DATA_SOURCE,
                                   'xiami_music_index2item_freq' + str(freq_n_item) + '_topu' + str(top_n_user))
    item2index_path = os.path.join(BASE_DIR, DATA_SOURCE,
                                   'xiami_music_item2index_freq' + str(freq_n_item) + '_topu' + str(top_n_user))
    index2length_path = os.path.join(BASE_DIR, DATA_SOURCE,
                                     'xiami_music_index2length_freq' + str(freq_n_item) + '_topu' + str(top_n_user))
    index2words_path = os.path.join(BASE_DIR, DATA_SOURCE,
                                    'xiami_music_index2words_freq' + str(freq_n_item) + '_topu' + str(top_n_user))

    item_length_path = os.path.join(BASE_DIR, DATA_SOURCE, 'xiami_music_item_length_freq' + str(freq_n_item))
    ...
    # 首行
    out_tr_baseline_uit.write(str(top_n_user) + ", " + str(baseline_config.top_n_item_dict_xiami[freq_n_item]) + '\n')
    ...
    #首行
    out_tr_lstm_uit.write(str(top_n_user) + ", " + str(baseline_config.top_n_item_dict_xiami[freq_n_item]) + '\n')
    ...
    if os.path.exists(index2item_path) and os.path.exists(item2index_path):
        ...
        # -------------------------------------------------------------------------------------
        # csv格式：['user_id', 'song_id', 'timestamp', 'device', 'song_name', 'artist_name'],
        musicInfo_file = open('../datasets/xiami/id_words.txt', 'r')
        lines=musicInfo_file.readlines()
        ...
        for line in lines:
            ...
            list2=list1[1].split(' ')
            musicId=list1[0]
            if musicId not in item_index2item_set:#剔除无关音乐信息
                continue
            artistId=list2[0]
            albumId=list2[1]
        ...
        # 音乐信息-index
        artistID = dict((v, i + len(item_index2item)) for i, v in enumerate(artist_list))#id-index
        ...
        music2artistFile = os.path.join(BASE_DIR, DATA_SOURCE,
                                        'xiami_music_index2artist_freq' + str(freq_n_item) + '_topu' + str(top_n_user))
        pickle.dump(music2artist_res, open(music2artistFile, 'wb'))
        music2albumFile = os.path.join(BASE_DIR, DATA_SOURCE,
                                       'xiami_music_index2album_freq' + str(freq_n_item) + '_topu' + str(top_n_user))
    ...
    for user_id, length in user_group.size().sort_values().iteritems():
        ...
        if len(new_item_seq) < min_length:  # 跳过过滤之后的交互记录少于min_length的用户
            ...
            user_index_baseline = user_index - baseline_config.top_n_item_dict_xiami[freq_n_item]
...
if __name__ == '__main__':
    ...
    # for freq_n_item in [10, 20, 30, 40, 50]:
